...
@section('content')
    ...
    <table class="w-full text-left transition-opacity ease-in-out duration-100">
        ...
        <tr class="text-black">
            ...
            <th class="font-normal px-3 pt-0 pb-3 border-b border-gray-200 dark:border-gray-800">Odbiory</th>
            <th class="font-normal px-3 pt-0 pb-3 border-b border-gray-200 dark:border-gray-800">Rewizje</th>
            <th class="font-normal px-3 pt-0 pb-3 border-b border-gray-200 dark:border-gray-800">Wypożyczenia</th>
        ...
        @foreach($raporty as $raport)
                            ...
                            <div id="dropdown{{$raport->id}}" class="drop-down">
                                ...
                                <div class="drop-down__menu-box">
                                    <ul class="drop-down__menu">
                                        <li data-name="profile" class="drop-down__item" data-modal-target="editRecordModal{{$raport->id}}" data-modal-toggle="editRecordModal{{$raport->id}}">Edytuj</li>
                                        <li data-name="role" class="drop-down__item" data-modal-target="roleModal{{$raport->id}}" data-modal-toggle="roleModal{{$raport->id}}">Rola</li>
            ...
            <!-- Edit modal -->
            <div id="editRecordModal{{$raport->id}}" tabindex="-1" aria-hidden="true" class="editRecordModal fixed top-0 left-0 right-0 z-50 hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full">
                <div class="relative w-full max-w-2xl">
                    ...
                    <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                        ...
                        <div class="modal-body p-4">
                            <form action="{{ route('employees.update', $raport->id) }}" method="POST">
                                @csrf
                                @method('PUT')

                                <div class="mb-4">
                                    <label for="name" class="block text-gray-800 dark:text-white font-semibold mb-1">Nazwa</label>
                                    <input type="text" class="form-control w-full border-gray-300 rounded-lg focus:ring-blue-300" id="name" name="name" value="{{ $raport->name }}" required>
                                </div>

                                <div class="mb-4">
                                    <label for="email" class="block text-gray-800 dark:text-white font-semibold mb-1">Email</label>
                                    <input type="email" class="form-control w-full border-gray-300 rounded-lg focus:ring-blue-300" id="email" name="email" value="{{ $raport->email }}" required>
                                </div>

                                <div class="mb-4">
                                    <label for="salary" class="block text-gray-800 dark:text-white font-semibold mb-1">Pensja</label>
                                    <input type="number" class="form-control w-full border-gray-300 rounded-lg focus:ring-blue-300" id="salary" name="salary" value="{{ $raport->salary }}" required>
                                </div>
                                ...
                                <div class="mb-4">
                                    ...
                                    <select class="form-select w-full border-gray-300 rounded-lg focus:ring-blue-300" id="id_placowki" name="id_placowki" required>
                                        ...
                                        @foreach(\App\Models\Placowki::all() as $placowka)
                                            <option value="{{ $placowka->id }}" {{ $placowka->id == $raport->id_placowki ? 'selected' : '' }}>
                                                {{ $placowka->nazwa }}
                                            </option>
                                        @endforeach
                                    </select>
            ...
            <div id="roleModal{{$raport->id}}" class="modal fixed inset-0 flex items-center justify-center z-50 hidden">
                ...
                <div class="modal-container bg-white w-full mx-auto rounded shadow-lg z-50 max-w-md">
                    <div class="modal-content py-4 text-left px-6">
                        ...
                        <!-- Modal body -->
                        <div class="my-4">
                            <form action="{{ route('pracownicy.changerole', $raport->id) }}" method="POST">
                                ...
                                <div class="mb-4">
                                    ...
                                    <select class="form-select w-full border-gray-300 rounded-lg focus:ring-blue-300" id="role" name="role" required>
                                        ...
                                        <option value="admin" {{ $raport->role === 'admin' ? 'selected' : '' }}>Admin</option>
                                        <option value="employee" {{ $raport->role === 'employee' ? 'selected' : '' }}>Employee</option>
                                        <option value="client" {{ $raport->role === 'client' ? 'selected' : '' }}>Client</option>
                                    </select>
                                </div>
